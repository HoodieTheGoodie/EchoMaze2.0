...
export function bossExplosion(cx, cy, radius, currentTime) {
  ...
  if (src !== 'mortar') {
    ...
    for (const o of (gameState.enemies || [])) {
      ...
      // SECRET: Bazooka Mode - stun enemies instead of killing them (6-7 seconds)
      if (src === 'rocket' && isBazookaMode()) {
        const stunDuration = 6000 + Math.random() * 1000; // 6-7 seconds
        o._frozenUntil = currentTime + stunDuration;
        console.log('[bazooka mode] stunned', o.type, 'for', Math.floor(stunDuration/1000), 'seconds');
        keep.push(o);
        continue;
      }
