...
export class ParticleSystem {
    ...
    render(ctx, currentTime) {
        for (const p of this.particles) {
            ...
            if (p.shape === 'hexagon') {
                ...
                ctx.closePath();
                ctx.fill();
            } else if (p.shape === 'player-ghost') {
                // Draw afterimage circle (simplified player)
                ctx.beginPath();
    ...
    // Shield activation - hexagon particles spiral out
    spawnShieldActivation(x, y) {
        for (let i = 0; i < 12; i++) {
    ...
    // Shield shatter - geometric shards fly outward
    spawnShieldShatter(x, y) {
        for (let i = 0; i < 20; i++) {
            ...
            this.particles.push({
                ...
                decay: 0.03,
                size: 3 + Math.random() * 3,
                color: `rgba(0, 246, 255, ${0.8 + Math.random() * 0.2})`,
                shape: 'shard',
                rotation: angle,
                rotationSpeed: 0.15 * (Math.random() > 0.5 ? 1 : -1),
                gravity: 0.15
            });
        }
    }
    ...
    spawnDashAfterimage(x, y, direction) {
        const angle = Math.atan2(direction.dy, direction.dx);
        for (let i = 0; i < 5; i++) {
            this.particles.push({
                x: x + (Math.random() - 0.5) * 15,
                y: y + (Math.random() - 0.5) * 15,
                vx: -Math.cos(angle) * (1 + i * 0.5),
                vy: -Math.sin(angle) * (1 + i * 0.5),
                life: 1.0,
                decay: 0.08,
                size: 8 - i,
                color: `rgba(0, 246, 255, ${0.6 - i * 0.1})`,
                shape: 'player-ghost'
            });
        }
    }
